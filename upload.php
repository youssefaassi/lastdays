...
if ((isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : '')) == 'simple') {
    ...
    // then we check if the file has been uploaded properly
    // in its *temporary* location in the server (often, it is /tmp)
    if ($handle->uploaded) { // ON EST ICIIIIIIIIIIIIIII
        ...
        if ($handle->processed) {
			...
			$_POST["nomObjet"] = "";
			$_POST["description"] = "";
			$_SESSION["bugNomObjet"] = "";
			$_SESSION["bugDescription"] = "";
			include 'ajout.image.view.php';
            echo '<a href="'.$dir_pics.'/' . $handle->file_dst_name . '" class="filedata">  Fichier:' . $handle->file_dst_name . ' - ' . round(filesize($handle->file_dst_pathname)/256)/4 . 'KB</a>';
			$_SESSION["img"] = $dir_pics.'/' . $handle->file_dst_name;
            echo '  <img src="'.$dir_pics.'/'.$handle->file_dst_name.'"/>';
            // echo '   (' . round(filesize($handle->file_dst_pathname)/256)/4 . 'KB)';
            echo '</p>';
        } else {
            // one error occured
            echo '<p class="result">';
            echo '  <b>Le fichier n\'a pas été téléchargé à l\'emplacement desiré.</b><br />';
            echo '  Error: ' . $handle->error . '';
            echo '</p>';
        }
        ...
        // i.e. the server didn't receive the file
        echo '<p class="result">';
        echo '  <b>Le fichier n\'a pas été téléchargé dans le serveur.</b><br />';
        echo '  Error: ' . $handle->error . '';
        echo '</p>';
    }
